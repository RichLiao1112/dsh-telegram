...
- Files delivered through Web's `present` tool are also sent to Telegram; videos up to 50 MB use Telegram's native video message, while larger files are sent as 49 MB document parts.
...
- A command result that identifies richer output reports the session event that owns it; Telegram does not render other rich output besides `present` file deliveries. Files above 50 MB are split into parts.
